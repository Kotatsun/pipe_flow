...
integer, parameter:: n=20, nx=n*10, ny=n, nz=n                                          ! 格子数n
real(8) :: p(0:nx+1,0:ny+1,0:nz+1)                                                    & ! p:圧力
          ,u(0:nx,0:ny+1,0:nz+1),v(0:nx+1,0:ny,0:nz+1),w(0:nx+1,0:ny+1,0:nz)          & ! u,v:速度
          ,phi(0:nx+1,0:ny+1,0:nz+1),divup(1:nx,1:ny,0:nz+1)                          & ! phi: 補正圧力, divup: 予測速度の発散
          ,up(0:nx,0:ny+1,0:nz+1),vp(0:nx+1,0:ny,0:nz+1),wp(0:nx+1,0:ny+1,0:nz)       & ! up, vp: 予測速度
          ! ,psi(0:nx,0:ny+1,0:nz+1)                                                    & ! psi: 流れ関数
...
do i=0,nx
  p(i,ny+1,nz+1)=0.d0
  u(i,ny+1,nz+1)=0.d0
  the(i,ny+1,nz+1)=0.d0
enddo
do j=0,ny
  p(nx+1,j,nz+1)=0.d0
  v(nx+1,j,nz+1)=0.d0
  the(nx+1,j,nz+1)=0.d0
enddo
do k=0,nz
  p(nx+1,ny+1,k)=0.d0
  w(nx+1,ny+1,k)=0.d0
  the(nx+1,ny+1,k)=0.d0
enddo
p(nx+1,ny+1,nz+1)=0.d0
the(nx+1,ny+1,nz+1)=0.d0
...
!     lower wall (south)
  do i=0,nx
    do k=0,nz
      ...
      v(i,0,k)=vs
      p(i,0,k)=p(i,1,k)
      the(i,0,k)=the(i,1,k)+q_w*dy*dz !heat flux constant
!     upper wall (north)
      w(i,ny+1,k)=2.*wn-w(i,ny,k)
      u(i,ny+1,k)=2.*un-u(i,ny,k)
      v(i,ny,k)=vn
      p(i,ny+1,k)=p(i,ny,k)
      the(i,ny+1,k)=the(i,ny,k)+q_w*dy*dz !heat flux constant
    enddo
  enddo
!     forward wall (forward)
  do i=0,nx
    do j=0,ny
      ...
      v(i,j,0)=vf
      p(i,j,0)=p(i,j,1)
      the(i,j,0)=the(i,j,1)+q_w*dy*dz !heat flux constant
!     backward wall (backward)
      w(i,j,nz)=wb
      u(i,j,nz+1)=2.*ub-u(i,j,nz)
      v(i,j,nz+1)=vb
      p(i,j,nz+1)=p(i,j,nz)
      the(i,j,nz+1)=the(i,j,nz)+q_w*dy*dz !heat flux constant
    enddo
  enddo
