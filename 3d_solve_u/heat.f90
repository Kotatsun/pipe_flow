...
integer,parameter:: stepmax=1000000                                                      ! 時間ステップ数
integer:: step                                                                          ! 時間ステップ
integer :: idf
! grid size 
dy=0.5d0/6.d0 
...
!$OMP end parallel do
  ...
  if(mod(step, 5000) == 0) then
    ...
    write (filename, '("output/flow", i7.7, ".txt")') step
    ...
    do j=1, ny
      do i=1, nx
        ...
        write(11,'(2f20.15, 1x, 2f20.15)') x0, y0, u0, thermo, step*dt*0.06d0, (maxval(the)+1.d0)*310.d0
      end do
      write(11,*)
    end do
    close(11)
